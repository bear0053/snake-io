...
2. Check specifically for:
   ...
   - **Leftover debug/test scaffolding**: grep for `window.__debug`, stray `console.log`, or other ad-hoc instrumentation that may have been left in `js/main.js` after a testing session (see the `run` skill — this pattern is a deliberate, temporary testing aid that must never ship).
...
4. **Verify nothing broke.** This codebase has no automated test suite, so after cleanup: serve the game and run the `run` skill's smoke test (`.claude/skills/run/examples/smoke_test.py`), and spot-check any area you touched (e.g. if you touched a specific level's rendering, load that level and screenshot it — see the `run` skill for the debug-hook pattern for jumping straight to a level).
...
6. Summarize what was removed/simplified and why, whether the README needed updates, and confirm the smoke test passed, before considering the pass done.
